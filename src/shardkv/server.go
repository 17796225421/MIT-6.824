...
func (kv *ShardKV) Execute(command Command, response *CommandResponse) {
	// 1. 使用raft的开始，传入命令请求，传出新日志索引、标记是否领导者
	// 2. 如果标记不是领导者，直接返回
	// 3. 利用新日志索引从命令响应管道数组找到命令响应管道
	// 4. select监视命令响应管道和超时管道
	// 5. 如果命令响应管道，得到命令响应
	// 6. 如果超时管道，设置命令响应的错误
	// 7. 创建协程，删除命令响应管道数组的命令响应管道
...
func (kv *ShardKV) applier() {
	...
	// 5. 从应用消息获得命令请求
	// 6. 如果命令请求的命令类型是读写操作日志，使用kv服务器的应用读写操作日志。
	// 7. 如果命令请求的命令类型是配置更新日志，使用kv服务器的应用配置更新日志
	// 8. 如果命令请求的命令类型是分片更新日志，使用kv服务器的应用分片更新日志
	// 9. 如果命令请求的命令类型是分片删除日志，使用kv服务器的应用分片删除日志
	// 10. 如果命令请求的命令类型是空日志，使用kv服务器的应用空日志
	// 11. 使用raft的获得状态，传出当前任期和标记是否领导者，如果标记是领导者且应用消息的命令任期等于当前任期，利用应用请求的命令索引从命令响应管道数组找到命令响应管道，将命令响应交给命令响应管道。
	// 12. 快照？
